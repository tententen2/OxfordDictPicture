...
class DictDetailRepository @Inject constructor(
    ...
    private val mapperImage:DetailDataImageMapper,
    private val factory:DetailDataStoreFactory
):DetailRepository {

    override fun getDetailImage(query: String): Single<DataDictImagePresent> {
        val dataStore = factory.retrieveDetailData()
        return dataStore.getDetailImage(query)
            .flatMap {
                if(dataStore is DetailDataCacheStore){
                    Single.just(mapperImage.mapFromEntity(it))
                }else{
                    factory.saveData().toSingle { mapperImage.mapFromEntity(it) }
                }
            }
    }
